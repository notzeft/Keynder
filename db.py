...
class Database:
    ...
    def create_db(self):
        certs_table = '''CREATE TABLE certs(pub_key TEXT PRIMARY KEY NOT NULL,
        signature TEXT,
        issuer TEXT,
        validity TEXT,
        ...
        keys_table = '''CREATE TABLE keys(id INTEGER PRIMARY KEY NOT NULL,
    ...
    def insert_many(self,keys):
        ...
        query = '''INSERT INTO keys(data,type,algo,size,format) VALUES(?,?,?,?,?);'''
    ...
    def insert_key(self,key):
        ...
        size = key.taille
        form = '\''+key.form+'\''
        tab = [(data,tipe,algo,size,form,pub_key)]
        query = '''INSERT INTO keys(pub_key,key,type,algo,size,format) VALUES({},{},{},{},{},{});'''.format(pub_key,data,tipe,algo,size,form)
        print(query)
        try:
            self.db.executescript(query)
    ...
    def insert_cert(self,cert):
        ...
        query = '''INSERT INTO certs(pub_key,signature,issuer,validity,format,certificate) VALUES({},{},{},{},{},{});'''.format(pub_key,signature,issuer,validity,form,certificate)
        try:
            self.db.executescript(query)
        except Exception as e:
            print(e)
    ...
    def match_cert_key(self):
        results = self.db.execute('''SELECT keys.id,keys.key,certs.pub_key FROM keys,certs WHERE keys.type=\'Pri\' AND certs.pub_key=keys.pub_key;''')
        for res in results:
            update_query = '''UPDATE certs SET pub_key_id={} WHERE certs.pub_key={};'''.format(res[0],'\''+res[2]+'\'')
            for r in res:
                print(r)
    ...
    def export_matches(self,output_file):
        query = '''SELECT keys.id,keys.pub_key,certs.pub_key FROM keys,certs WHERE keys.type=\'Pri\' AND certs.pub_key=keys.pub_key;'''
        command = '''sqlite3 -header -csv {} {} > {}'''.format(self.filename,'"'+query+'"',output_file)
        print(command)
        os.system(command)
