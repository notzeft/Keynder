...
class StringGrabber():
    ...
    def ini_listFiles(self, list_files, directory):
        """ Initialise la liste_filesavec les noms des fichiers (directory et
        subdirectory recursivement).
        """
    ...
    def unzip(self, file, extract_location):
        """ unzip un file zip et mets les extract dans extract_location. """
        zip_ref = zipfile.ZipFile(file, 'r')
        zip_ref.extractall(extract_location)
        zip_ref.close()
        os.system("rm -rf " + file)

    def extract_lines(self, file, directory, list_data):
        """ Traite les fichiers binaires: elle copie le contenu du fichier
        binaire dans temp.txt pui dans temp.txt elle met chaque cle ou
        certificat dans la liste finale.
        """
        all = os.popen('strings ' + file + ' 2>/dev/null').read()

        with open(directory + "temp.txt", "a", encoding='utf-8',
                  errors='ignore') as file_bin:
            file_bin.write(str(all).rstrip("\n"))

        with open(directory + "temp.txt", "r", encoding='utf-8',
                  errors='ignore') as file_bin1:
            data = file_bin1.readlines()
            string = ""
            begin_data = ["-----BEGIN PUBLIC KEY-----",
                          "-----BEGIN EC PRIVATE KEY-----",
                          "-----BEGIN RSA PRIVATE KEY-----",
                          "-----BEGIN CERTIFICATE-----"]
            end_data = ["-----END PUBLIC KEY-----",
                        "-----END EC PRIVATE KEY-----",
                        "-----END RSA PRIVATE KEY-----",
                        "-----END CERTIFICATE-----"]
            for line in data:
    ...
    def der_file(self, list_files, list_data):
        """ Pour chaque ficher, on met son contenu dans la liste finale. """
        files = {}
    ...
    def DataFiles_to_DataList(self, fullpath):
        ...
        # Dossier dans lequel il y'a tout les fichier de cles: bin, zip, txt,
        # der, sous-repertoires, etc
        dir = fullpath
        ...
        # filenames est reinitialise avec les nouveaux fichiers
        self.ini_listFiles(filenames, dir_create)
